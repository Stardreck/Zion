...
class Bubble:
    ...
    pos: position as a pygame Vector2.
    radius: bubble radius.
    color: RGB color.
    lifetime: how long the bubble stays visible.
    age: time elapsed since the bubble spawned.
    """
...
class BubblePopChallengeMiniGame(MiniGame):
    ...
    A countdown timer shows the remaining time (default 30 seconds).
    After the time is up, no new bubbles will spawn.
    """

    def __init__(self, game: StoryGame, background: str, game_duration: float = 30.0, initial_spawn_interval: float = 1.5):
    ...
    def _build_ui(self) -> None:
        ...
        timer_rect = Rect(700, 10, 200, 50)
        self.timer_label = UILabel(
            relative_rect=timer_rect,
            text=f"Time: {int(self.game_duration)}",
            manager=self.game.ui_manager.gui_manager,
    ...
    def update(self, delta_time: float) -> None:
        ...
        if self.timer_label:
            ...
            self.timer_label.set_text(f"Time: {remaining}")
