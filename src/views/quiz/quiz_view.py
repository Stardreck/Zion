from __future__ import annotations
import pygame
import pygame_gui
from pygame import Rect
from src.components.ui.ui_button import UIButton
from src.components.ui.ui_label import UILabel
from src.components.ui.ui_text_box import UITextBox
from src.components.ui.ui_text_entry_line import UITextEntryLine
from src.components.ui.ui_panel import UIPanel
from src.views.view import View
from src.models.quiz import Quiz

from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from src.games.story_game import StoryGame


class QuizView(View):
    """
    Displays a quiz with a question, a list of answer options and an input field.
    It contains its own game loop until the user confirms their answer.
    """
    def __init__(self, game: StoryGame, quiz: Quiz):
        """
        Initialize the QuizView.

        :param game: The main StoryGame instance.
        :param quiz: The Quiz object containing quiz data.
        """
        super().__init__(game.ui_manager.gui_manager)
        self.game: StoryGame = game
        self.quiz: Quiz = quiz
        self.is_running: bool = True

        # Load the background image (from planet.background_image)
        self.background_image = pygame.image.load(self.game.engine.config.main_menu_background_image).convert()

        # UI element placeholders
        self.panel_bg: UIPanel | None = None
        self.panel: UIPanel | None = None
        self.title: UILabel | None = None
        self.question: UILabel | None = None
        self.answer_list: UITextBox | None = None
        self.input_label: UILabel | None = None
        self.input_info_label: UILabel | None = None
        self.input_field: UITextEntryLine | None = None
        self.confirm_button: UIButton | None = None

    def build_ui(self):
        """
        Create and arrange all UI elements for the quiz.
        The layout is composed of:
          - A background panel (a transparent container centered in the window)
          - An inner panel for quiz content (question, answers, input)
          - A confirm button at the bottom.
        """
        window_width = self.game.engine.width
        window_height = self.game.engine.height

        # Create the background panel (centered container)
        self.panel_bg = UIPanel(
            relative_rect=Rect(0, 0, 550, 480),
            manager=self.pygame_gui_ui_manager,
            anchors={"center": "center"},
            object_id="default_panel_transparent"
        )

        # Create the inner panel for quiz content
        self.panel = UIPanel(
            relative_rect=Rect(0, 10, 530, 400),
            manager=self.pygame_gui_ui_manager,
            container=self.panel_bg,
            anchors={"centerx": "centerx", "top": "top"},
            object_id="default_panel"
        )

        # Title label: "Quiz-Frage"
        self.title = UILabel(
            relative_rect=Rect(0, 0, 500, 50),
            manager=self.pygame_gui_ui_manager,
            text="Quiz-Frage",
            container=self.panel,
            anchors={"centerx": "centerx", "top": "top"},
            object_id="quiz_title"
        )

        # Question label (displaying the quiz question)
        self.question = UILabel(
            relative_rect=Rect(0, 50, 500, 30),
            manager=self.pygame_gui_ui_manager,
            text=self.quiz.question,
            container=self.panel,
            anchors={"centerx": "centerx", "top": "top"},
            object_id="quiz_question"
        )

        # Answer list as HTML (each option in a new line)
        answers_html = "<br/>".join(
            [f"<b>{index + 1})</b> {answer}" for index, answer in enumerate(self.quiz.answer_options)]
        ) if self.quiz.answer_options else "Keine Antworten vorhanden"
        self.answer_list = UITextBox(
            relative_rect=Rect(16.5, 100, 200, 200),
            manager=self.pygame_gui_ui_manager,
            html_text=answers_html,
            container=self.panel,
            anchors={"left": "left", "top": "top"},
            object_id="quiz_answer_list"
        )

        # Input label for the answer field
        self.input_label = UILabel(
            relative_rect=Rect(16.5, -60, 150, 50),
            manager=self.pygame_gui_ui_manager,
            text="Antwort eingeben:",
            container=self.panel,
            anchors={"left": "left", "bottom": "bottom"},
            object_id="quiz_input_label"
        )
        # Optionaler Informationstext zum Input (z. B. Hinweis auf gültige Eingaben)
        self.input_info_label = UILabel(
            relative_rect=Rect(16.5, -40, 150, 50),
            manager=self.pygame_gui_ui_manager,
            text="(1,2,3,4)",
            container=self.panel,
            anchors={"left": "left", "bottom": "bottom"},
            object_id="quiz_input_label_info"
        )
        # Input field for user answer
        self.input_field = UITextEntryLine(
            relative_rect=Rect(16.5 + 150, -60, 350, 50),
            manager=self.pygame_gui_ui_manager,
            container=self.panel,
            anchors={"left": "left", "bottom": "bottom"},
            object_id="quiz_input"
        )

        # Confirm button at the bottom of the background panel
        button_rect = Rect(0, -60, 500, 50)
        self.confirm_button = UIButton(
            relative_rect=button_rect,
            manager=self.pygame_gui_ui_manager,
            text="Bestätigen",
            container=self.panel_bg,
            anchors={"centerx": "centerx", "bottom": "bottom"},
            object_id="panel_button"
        )
        self.confirm_button.bind(pygame_gui.UI_BUTTON_PRESSED, lambda event: self.submit())

    def run(self):
        """
        Display the quiz view in its own game loop until the user confirms their answer.
        """
        self.build_ui()
        clock = pygame.time.Clock()
        self.is_running = True
        while self.is_running:
            time_delta = clock.tick(self.game.engine.fps) / 1000.0
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.game.stop()
                    self.is_running = False
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_d:
                        self.game.debug_manager.toggle_debug_mode()
                self.pygame_gui_ui_manager.process_events(event)
            self.pygame_gui_ui_manager.update(time_delta)
            self.game.window.blit(self.background_image, (0, 0))
            self.pygame_gui_ui_manager.draw_ui(self.game.window)
            pygame.display.update()
        self.kill()

    def submit(self):
        """
        Process the user's input and pass it to the quiz manager.
        If no text is entered, the submission is ignored.
        """
        user_answer: str = self.input_field.get_text().strip()
        if len(user_answer) == 0:
            return
        self.game.quiz_manager.process_submit(self.quiz, user_answer)
        # Optionally: Remove focus from UI elements
        self.pygame_gui_ui_manager.set_focus_set(None)
        self.is_running = False

    def kill(self):
        """
        Clean up and remove all UI elements.
        """
        if self.panel_bg:
            self.panel_bg.kill()
        if self.title:
            self.title.kill()
        if self.question:
            self.question.kill()
        if self.answer_list:
            self.answer_list.kill()
        if self.input_label:
            self.input_label.kill()
        if self.input_info_label:
            self.input_info_label.kill()
        if self.panel:
            self.panel.kill()
        if self.confirm_button:
            self.confirm_button.kill()
